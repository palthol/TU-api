...
- `supabase/migrations/0024_generate_sessions_rpc.sql` (idempotent SQL; not applied to production; numbered `0024` because API-AUTH-001 already took `0023` on `develop`)
...
| `npm --workspace services/api run test` (after merging `origin/develop` / API-AUTH-001) | 0 | 14 files / 109 tests. Log: `/opt/cursor/artifacts/api_sched_001_merge_vitest.log` |
| `npm run guard:waiver-schema` (post-merge) | 0 | Log: `/opt/cursor/artifacts/api_sched_001_merge_waiver_guard.log` |
...
- Migration `0024` is in-repo only (idempotent unique index + `create or replace` RPC; `service_role` execute only). `0023` on `develop` is staff RBAC (API-AUTH-001).
...
`done`. Residual risk: generate-sessions against a live database needs migration `0024` applied in a later authorized schema push. Until then the RPC and unique index are absent in production (currently 0 sessions). Vitest uses an in-memory Supabase stand-in; the SQL was not executed against production or a local Supabase. Template edits do not rewrite already-generated sessions.

Merge note (2026-09-14): `origin/develop` gained API-AUTH-001 (`0023_staff_rbac.sql`). The only Git conflict was `docs/current-state.md` (simple: keep both AUTH RBAC and SCHED template facts). The dual `0023` filenames were resolved by renaming this task's SQL to `0024`.
