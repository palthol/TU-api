...
| **Cron / scheduler calls** | Discord notification routes and `POST /api/admin/billing/generate-monthly-charges` also accept header `x-cron-secret` when **`CRON_SECRET`** is set on the API (in addition to `x-admin-key`). Scheduler calls authenticate as actor `cron`. Discord retains its Render Cron runbook; monthly billing has selected a Cloudflare Worker scheduler, not yet implemented/deployed. |
