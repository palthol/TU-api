...
**Verified:** 2026-09-22 (live Supabase migration history and billing automation schema); 2026-09-21 (billing uniqueness scope and per-class conversion anchor; V1 automatic subscription charge generation); 2026-09-16 (Stripe webhook → `record_payment`; atomic record-payment RPC); 2026-09-14 (migration history filenames; Discord cron runbook; staff RBAC; schedule templates); 2026-09-05 (deploy inventory); production snapshot 2026-09-03
...
| Schema | verified | Project healthy; applied history is `0001`–`0020` + `20260608191715` | Repo filename now matches live version `20260608191715`. Pending in-repo: `20260914150818`, `20260914185843`, `20260914202053` (formerly `0022`–`0024`), `20260916174649` (`record_payment`), and `20260916225225` (`payment_processor_*`); not applied. Billing migrations `20260921185003` and `20260921221500` are applied. Production `schema_migrations` was not rewritten from this change. |
...
| Billing/receipts | verified (non-prod) | Local smoke (API-VAL-001): personal finance entries, charge discounts, record-payment, receipt void, refund. Billing pgTAP: 50/50 lifecycle assertions for initial/recurring generation, persisted opt-out baseline, status/cadence exclusions, idempotency, future periods, historical safety, same-day per-class charges, prorated upgrades on an existing coverage start, and paid per-class conversions. API-HARD-001: Vitest covers atomic `record_payment` RPC, idempotent retry, and missing-RPC fallback. API-PAY-001: Vitest covers Stripe signature verify, `payment_intent.succeeded` → `record_payment`, duplicate event replay, unmatched metadata, missing-RPC `503`, ignored `charge.succeeded`, and `refund.created` → `record_payment_refund` | Production has 0 subscriptions, charges, payments, and receipts as of 2026-09-22. Billing migrations `20260921185003` and `20260921221500` are applied; the daily Cloudflare billing Worker is in-repo but not yet deployed/configured. Live Stripe webhook is not registered. Pre-existing subscriptions must be reviewed individually; `automatic_billing_starts_at = NULL` disables recurring generation until an operator establishes a baseline. Admin record-payment still falls back to sequential inserts until `20260916174649` is applied; Stripe webhooks do not use that fallback |
...
- No route-level suites cover the main billing, subscription, scheduling, or reporting
  workflows.
- Monthly-charge generation is implemented in the database and invoked by the in-repo Cloudflare Worker at `workers/billing-cron/`. The Worker is not deployed/configured yet. Before backfilling, review every active paid monthly subscription and set its intentional `automatic_billing_starts_at` baseline; `NULL` keeps automation disabled.
...
- Live Supabase migration history reconciled on 2026-09-22: `20260921185003` and `20260921221500` are applied; the production snapshot still has 0 subscriptions, charges, payments, and receipts. Cloudflare billing Worker source and runbook added; deployment/configuration remains an operator action.
