...
| Schema | verified | Project healthy; applied history is `0001`–`0020` + `20260608191715` | Repo filename now matches live version `20260608191715`. Pending in-repo: `20260914150818`, `20260914185843`, `20260914202053` (formerly `0022`–`0024`), `20260916174649` (`record_payment`), `20260916225225` (`payment_processor_*`), `20260921185003` (billing automation safety), `20260921221500` (monthly-period uniqueness and conversion anchor); not applied. Production `schema_migrations` was not rewritten from this change. |
...
| Billing/receipts | verified (non-prod) | Local smoke (API-VAL-001): personal finance entries, charge discounts, record-payment, receipt void, refund. Billing pgTAP: 50/50 lifecycle assertions for initial/recurring generation, persisted opt-out baseline, status/cadence exclusions, idempotency, future periods, historical safety, same-day per-class charges, prorated upgrades on an existing coverage start, and paid per-class conversions. API-HARD-001: Vitest covers atomic `record_payment` RPC, idempotent retry, and missing-RPC fallback. API-PAY-001: Vitest covers Stripe signature verify, `payment_intent.succeeded` → `record_payment`, duplicate event replay, unmatched metadata, missing-RPC `503`, ignored `charge.succeeded`, and `refund.created` → `record_payment_refund` | Production still has 0 charges, payments, receipts. Migrations `20260916174649`, `20260916225225`, `20260921185003`, and `20260921221500` are in-repo and **not** applied. Live Stripe webhook is **not** registered. Daily billing cron is documented but **not** enabled. Apply `20260921221500` in the same reviewed push as `20260921185003` before enabling the cron. Pre-existing subscriptions remain automation-disabled (`automatic_billing_starts_at = NULL`) pending individual operator review. Admin record-payment still falls back to sequential inserts until the RPC exists; Stripe webhooks do **not** use that fallback |
...
- No route-level suites cover the main billing, subscription, scheduling, or reporting
  workflows.
- Monthly-charge generation has a daily Render Cron runbook but no live configured scheduler. Apply `20260921185003` and `20260921221500` together after the duplicate-charge preflight, and only then consider enabling the job. The follow-up limits uniqueness to `charge_kind = monthly_period` and anchors paid per-class-to-monthly conversions.
